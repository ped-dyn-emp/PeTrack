...
bool MarkerColor::isOverlappingHead(const MyEllipse& e) const
{
    if (hasHead())
...
bool MarkerColor::isInsideHead(const Vec2F& p) const
{
    if (hasHead())
...
int MarkerColor::isOverlappingSpots(const MyEllipse& e) const
{
...
int MarkerColor::isInsideSpots(const Vec2F& p) const
{
    for (int i = 0; i < mSpots.size(); ++i)
...
void MarkerColor::modifyHead(const MyEllipse &head)
{
...
void MarkerColor::modifySpot(int i, const MyEllipse &spot)
{
    if (i < mSpots.size())
    {
        // man koennte auch pruefen:
...
void MarkerColor::addSpot(const MyEllipse &spot)
{
    mSpots.append(spot);
    mSpotCount.append(1);
}
void MarkerColor::deleteSpot(int i)
{
    mSpots.removeAt(i);
    mSpotCount.removeAt(i);
}

void MarkerColor::modifyQuadrangle(const Vec2F v[4])
{
    if (mHasQuadrangle)
    {
...
void MarkerColor::organize(const Mat &img, bool autoWB)
{
    int i, j, k;
...
MyEllipse MarkerColor::getCenterSpot() const
{
    if (mCenterIndex == -1)
...
MyEllipse MarkerColor::getColorSpot() const
{
    if (mColorIndex == -1)
        return mHead; // good fallback ?
    else
...
void MarkerColor::draw(Mat &img) const
{
    int i;
...
bool MarkerColorList::mayAddEllipse(const Mat &img, const MyEllipse& e, bool blackInside)
{
    int i;
...
bool MarkerColorList::mayAddQuadrangle(const Vec2F v[4]) //Vec2F p1, Vec2F p2, Vec2F p3, Vec2F p4
{
...
void MarkerColorList::organize(const Mat &img, bool autoWB)
{
    int i, j, k, s;
//     for (i = 0; i < size(); ++i)
...
void MarkerColorList::draw(Mat &img) const
{
    for (int i = 0; i < size(); ++i)
        at(i).draw(img);
}

void MarkerColorList::toCrossList(QList<TrackPoint> *crossList, bool ignoreWithoutMarker) const
{
    Vec2F v1, v2;
