...
show markerID list
: Displays only the trajectories of the people whose code markerIDs are listed here.
The input format is the same as for `show only people list`.
It also can be used together with `show only people list`.
In that case, the combined trajectories of all specified persons will be shown.
